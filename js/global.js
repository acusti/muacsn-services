...
$('#services-search').on('submit', function() {
	...
	// Remove .pre-search class for any matching elements
	$('.pre-search').removeClass('pre-search');
...
_s.$results.on('results.removed', function() {
	...
	if (_s.$result_table.find('tr').length < 2)
		_s.$result_table.find('tr').after('<tr><td class="no-results" colspan="5">Aucun service trouvé correspondant aux critères de recherche. <i>No services found that match the search criteria.</i></td></tr>');

	_s.$results.addClass('deployed').prepend(_s.$result_table).animate({height: _s.$results.children().outerHeight()}, 200, function() {
		$(this).css('height', 'auto');
	});
	// Scroll to the results of the page (in timeout, to give time for page to load)
	setTimeout(function() {
...
// Add functionality for search-again link
$('.search-again').on('click', function() {
	var $target = $($(this).attr('href'));
	if (!$target.length)
		return true;

	$('html, body').animate({scrollTop: ($target.offset().top - 10) + 'px'}, 300);
	return false;
});